---
title: "A6:2021 | Vulnerable and Outdated Components (4) | Cycubix Docs"
layout: default
nav_order: 4
parent: "A6:2021 | Vulnerable and Outdated Components | Cycubix Docs"
grand_parent: "WebGoat Labs | Web Application Security Essentials (2021) (2021) (2021) | Cycubix Docs"
---
# A6:2021 | Vulnerable and Outdated Components (4) | Cycubix Docs

### Components are everywhere <a href="#components_are_everywhere" id="components_are_everywhere"></a>

WebGoat uses almost **200 Java and JavaScript** libraries. Like most Java applications, we use maven to manage our java dependencies and we employ the wild, wild west strategy for managing JavaScript.

#### Vulnerable components in WebGoat? <a href="#vulnerable_components_in_webgoat" id="vulnerable_components_in_webgoat"></a>

When this lesson was created WebGoat contained more than a dozen high security risks within it’s components. Most of these were not deliberate choices. How are developers supposed to track this information across the hundreds of components?

<figure><img src="../../../../.gitbook/assets/vulnerable components 2 (1).png" alt=""><figcaption></figcaption></figure>
